...
public class ZoneActivityPicBack {
	...
	public ZoneActivityPicBack(final ZoneActivity act, View root) {
		this.act = act;
		this.root = root;
		...
		for(int i=0;i<user.getImgCount();i++){
			View head = act.createView(R.layout.intro_nav_imgveiw);
			imgViews.add(head);
		}
		...
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				for(int i=0;i<user.getImgCount();i++){
					...
					String imgPath = act.getTestPath()+user.getId()+"_"+"0"+(i+1)+".jpg";
					
					Bitmap bitmap = act.readBitmapAutoSize(imgPath, img.getWidth(), img.getHeight());
		...
		public void onPageSelected(int page) {
			for (int i = 0; i < imgViews.size(); i++) {
				...
				if (page == i && view.getTag()!=null) {
					...
					Bitmap bitmap = act.readBitmapAutoSize(path, (int)intrImg.getWidth()*2, intrImg.getHeight()*2);
